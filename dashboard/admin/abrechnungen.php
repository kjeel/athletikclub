...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'satz_aendern') {
    ...
    if ($trainer_id && is_numeric($satz) && (float)$satz >= 0 && (float)$satz <= 100) {
        $db->prepare(
            'INSERT INTO trainer_profile (user_id, provisionssatz) VALUES (?, ?)
             ON DUPLICATE KEY UPDATE provisionssatz = VALUES(provisionssatz)'
        )->execute([$trainer_id, (float)$satz]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'abrechnung_erstellen') {
    ...
    $stmt = $db->prepare('SELECT * FROM umsatz_eintraege WHERE trainer_id = ? AND abgerechnet_id IS NULL');
    $stmt->execute([$trainer_id]);
    ...
    $umsatz_gesamt    = array_sum(array_column($kurs_positionen, 'summe')) + array_sum(array_column($manuelle_positionen, 'betrag'));
    $provisionsbetrag = round($umsatz_gesamt * $provisionssatz / 100, 2);

    try {
        $db->beginTransaction();

        $db->prepare(
            'INSERT INTO abrechnungen (trainer_id, zeitraum_von, zeitraum_bis, umsatz_gesamt, provisionssatz, provisionsbetrag, erstellt_von)
             VALUES (?, ?, ?, ?, ?, ?, ?)'
        )->execute([$trainer_id, $zeitraum_von, $zeitraum_bis, $umsatz_gesamt, $provisionssatz, $provisionsbetrag, $user['id']]);
        $abrechnung_id = (int)$db->lastInsertId();

        $pos_stmt = $db->prepare(
            'INSERT INTO abrechnung_positionen (abrechnung_id, typ, beschreibung, datum, betrag) VALUES (?, ?, ?, ?, ?)'
        );
        foreach ($kurs_positionen as $k) {
            $beschreibung = $k['titel'] . ' (' . (int)$k['bezahlte_teilnehmer'] . 'x bezahlt à ' . number_format((float)$k['preis'], 2, ',', '.') . ' €)';
            $pos_stmt->execute([$abrechnung_id, 'kurs', $beschreibung, date('Y-m-d', strtotime($k['start_datum'])), $k['summe']]);
        }
        foreach ($manuelle_positionen as $m) {
            $pos_stmt->execute([$abrechnung_id, 'manuell', $m['beschreibung'], $m['leistungsdatum'], $m['betrag']]);
        }

        $db->prepare(
            "UPDATE kurs_anmeldungen ka JOIN kurse k ON k.id = ka.kurs_id
             SET ka.abgerechnet_id = ?
             WHERE k.trainer_id = ? AND ka.bezahlt = 1 AND ka.abgerechnet_id IS NULL"
        )->execute([$abrechnung_id, $trainer_id]);

        $db->prepare(
            'UPDATE umsatz_eintraege SET abgerechnet_id = ? WHERE trainer_id = ? AND abgerechnet_id IS NULL'
        )->execute([$abrechnung_id, $trainer_id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'als_ausgezahlt') {
    ...
    $db->prepare("UPDATE abrechnungen SET status = 'ausgezahlt', ausgezahlt_am = NOW() WHERE id = ?")->execute([$abrechnung_id]);
        ...
        <table class="data-table">
            ...
            <tbody>
                <?php foreach ($trainer_liste as $t):
                    $offen = (float)$t['offener_kursumsatz'] + (float)$t['offener_manueller_umsatz'];
                    $provision = $offen * (float)$t['provisionssatz'] / 100;
                ?>
                <tr>
                    <td class="text-primary"><?= e($t['vorname'] . ' ' . $t['nachname']) ?></td>
                    <td><?= number_format($offen, 2, ',', '.') ?> €</td>
                    <td>
